...
Le script fonctionne en calculant le courant électrique produit par la cellule solaire sur une plage de valeurs de tension, en utilisant un modèle mathématique à cinq paramètres appelé "modèle à une diode", choisi pour son équilibre entre simplicité et précision. Ce programme utilise spécifiquement l'implémentation de ce modèle par De Soto (développée par De Soto, Klein et Beckman en 2006), qui permet de déterminer les cinq paramètres de la cellule à partir de sa température (T) et de l'irradiance solaire absorbée (G). À partir de ces résultats, le script détermine le [point de puissance maximale](https://fr.wikipedia.org/wiki/Rendement_d%27une_cellule_photovolta%C3%AFque#Point_de_puissance_maximale) du panneau, son [facteur de forme](https://fr.wikipedia.org/wiki/Rendement_d%27une_cellule_photovolta%C3%AFque#Facteur_de_remplissage) et le rendement. Les données sont ensuite visualisées à travers des courbes caractéristiques I(V) et P(V).
